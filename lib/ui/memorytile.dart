...
class MemoryTileGame extends StatefulWidget {
  @override
  _MemoryTileGameState createState() => _MemoryTileGameState();
}

class _MemoryTileGameState extends State<MemoryTileGame> with TickerProviderStateMixin {
  ...
  void checkAnswer() {
    ...
    // Show the wrong guess icon and then wait for a few seconds before showing the pattern again
    setState(() {
      if (wrongCount > 0) {
        playWrongSound();
        isWrongGuess = true;
        isCelebrating = false; // Ensure that celebration is set to false when showing wrong guess icon
       // currentLevel--;
      } else {
        playCorrectSound();
        isCelebrating = true;
        isWrongGuess = false; // Ensure that wrong guess is set to false when celebrating
        score += 10; // Increase score for correct guess
        //currentLevel++;
      }
    });

    // Delay the next steps after the user's guess
    Future.delayed(Duration(seconds: 2), () {
      setState(() {
        isCelebrating = false;
        isWrongGuess = false;
        attemptsLeft--; // Decrement attempts left for each guess
        if(wrongCount>0 && currentLevel>0)
          {
            currentLevel--;
          }
        else{
        ...
        if (attemptsLeft == 0) {
          ...
          showPatternAndStartTimer(); // Show the new pattern and start the timer after delay
        }
      });
    });
  }
  ...
  Widget buildTile(int row, int col) {
    double tileSize = 50; // Set a constant tile size
    ...
    if (row < patternGrid.length && col < patternGrid[row].length) {
      ...
      if (!isShowingPattern && isWrongGuess && !patternGrid[row][col] && inputGrid[row][col]) {
        // If it's not part of the pattern and the user clicked it incorrectly
        tileColor = Colors.red; // Change the color to red
      }
            ...
            return Transform(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: tileColor, // Use the updated tile color
                  border: Border.all(color: Colors.brown, width: 2.0), // Add brown border
                ),
                child: Center(
                  child: Text(
                    '',
